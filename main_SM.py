...
if __name__ == '__main__':
    ...
    if not os.path.isfile("SM.txt"):
        for item_file in file:
            flag+=1
    #        print item.strip()
            img = Image.Image(dir_name + item_file.strip())
            img_read = img.run()
            ...
            list = np.concatenate((ClbpS_result, ClbpM_result), axis=0)

            data = open("SM.txt","a")
            for item_clbp in list:
                data.write( "%s\n" % item_clbp )
            data.write("%s\n" %kelas)
            data.write("\n")
            data.close()
