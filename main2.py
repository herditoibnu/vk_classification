...
if __name__ == '__main__':
    ...
    if not os.path.isfile("datadikit.txt"):
        for item in file:
            flag+=1
    #        print item.strip()
...
#        print item.strip()
        ...
        if flag == 4:
            flag = 0
            kelas += 1
        file_training = open("datadikit.txt","r")
        line_count = 0
        ...
        for jarak in array_jarak:
            ...
            cnt[jarak[1]]+=1
            if jarak[1] == kelas_testing[conter-1]:
                jumlah_benar +=1
            if(i==nilai_k):
                break
            i += 1
    ...
    print jumlah_benar
    print conter
    print (float(jumlah_benar)/float(conter)) * 100
    #print akurasi(kelas_training,hasil)